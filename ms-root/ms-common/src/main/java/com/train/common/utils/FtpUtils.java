...
public class FtpUtils {
    ...
    public final static String  BYTENCODING="UTF-8";
    ...
    */
     ...
     * @param ftpPath   FTP服务器中文件所在路径 格式，例：ftptest/aa
     * @param localPath 下载到本地的位置 格式，例：H:/download
     * @param fileName  文件名称，例：1.jpg
     */
...
//            byte2File(input2byte, localPath, fileName);
    ...
    /**
      ...
      * @return
      */
    public static boolean checkExist(FTPClient ftpClient, String ftpPath, String fileName){
        try {
            //进入文件所在目录，注意编码格式，以能够正确识别中文目录
            ftpClient.changeWorkingDirectory(new String(ftpPath.getBytes(BYTENCODING),FTP.DEFAULT_CONTROL_ENCODING));
            InputStream is = ftpClient.retrieveFileStream(new String(fileName.getBytes(BYTENCODING),FTP.DEFAULT_CONTROL_ENCODING));
    ...
    public static void main(String[] args){
        String url = "10.1.36.201";
        int port = 21;
        String username = "test";
        String password = "test";
        String fileName="1.jpeg";
        String basePath = "imgs";
        String filePath = "20190220";

        String localPath = "D:\\Life\\Pictures";
        File file = new File(localPath+File.separator+fileName);
        try{
            FileInputStream in = new FileInputStream(file);
            FTPClient ftpClient = FtpUtils.getFTPClient(url,port,username,password);
            boolean uploadIsSuccess = FtpUtils.uploadFile(ftpClient,basePath,filePath,fileName,in);
            System.out.println("upload file, result="+uploadIsSuccess);
        }catch (FileNotFoundException e){
            e.printStackTrace();
        }
    }
